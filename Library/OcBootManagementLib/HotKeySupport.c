  ...
  )
{
  ...
  DownkeyHandlerStr = OC_BLOB_GET (&Input->DownkeyHandler);
  if (AsciiStrCmp (DownkeyHandlerStr, "Enabled") == 0) {
    mUseDownkeys = TRUE;
  } else if (AsciiStrCmp (DownkeyHandlerStr, "Disabled") == 0) {
    mUseDownkeys = FALSE;
  } else if (AsciiStrCmp (DownkeyHandlerStr, "Auto") != 0) {
    DEBUG ((DEBUG_WARN, "OCHK: Invalid downkey handler mode %a\n", DownkeyHandlerStr));
  }
